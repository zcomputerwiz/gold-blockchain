...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in GOLD_ROOT.
    GOLD_ROOT_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within GOLD_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_chia(args, GOLD_ROOT_path)
    if args.plotter == "madmax":
        plot_madmax(args, GOLD_ROOT_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, GOLD_ROOT_path, root_path)
