...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "gl"), encode_puzzle_hash(new_ph_2, "gl"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "gl"))
            ...
            assert config["farmer"]["gl_target_address"] == encode_puzzle_hash(new_ph, "gl")
            assert config["pool"]["gl_target_address"] == encode_puzzle_hash(new_ph_3, "gl")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "gl")
