...
class TestPoolWalletRpc:
    ...
    @pytest.mark.asyncio
    async def test_absorb(self, one_wallet_node_and_rpc):
        client, wallet_node_0, full_node_api = one_wallet_node_and_rpc
        wallet_0 = wallet_node_0.wallet_state_manager.main_wallet
        our_ph = await wallet_0.get_new_puzzlehash()
        summaries_response = await client.get_wallets()
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            leave_pool_tx: TransactionRecord = await api_user[0].pw_self_pool(
                {"wallet_id": wallet_id}
            )
            ...
            await time_out_assert(timeout=20, function=status_is_leaving)
